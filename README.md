...
4. 标签体系学习了 Flomo APP、Flomo 从理念到体验都真的是很棒的设计、也推荐大家去用；但大家会很快发现 Anyhark 的定位完全不同，这部分见2.
5. Anyhark 会面向 Agent 设计能力，例如下一步所需要的备份功能，我希望 Agent 独立完成，只在必要时申请你的协助；--“用户变了，是时候重新看待产品使用成本了”

## 当前核心功能

一个桌面端的上下文管理应用（尽管初版像一款笔记软件），核心功能：

- **连接本地Agent** — 一键复制给本地的Openclaw🦞，让它学会使用
- **随手记** — 打开就写，用 `#标签` 归类，支持多级标签（比如 `#工作/周报`）
- **富文本** — 加粗、高亮、列表、@笔记/网址、检索、插入图片，够用就好
- **历史版本** — 每次修改自动存档，最多保留 10 个版本，随时可以回退
- **导入导出** — 支持从 Flomo、苹果备忘录迁移过来；也可以导出为 CSV 或 JSON 备份
- **回收站** — 删错了可以找回来
- **全文搜索** — 按关键词或标签检索

最简使用方式：复制本页链接直接发给你的本地 Agent 学习即可
...
- **苹果备忘录**（macOS）— 直接从备忘录 App 读取，无需手动导出。导入后原文中的 `#` 会转为 `+` 以避免标签冲突，每条笔记自动添加 `#Apple Notes/文件夹/前5字` 标签，可配合 AI Agent 进一步整理。
